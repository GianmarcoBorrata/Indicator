...
#'
#' Evaluate different methods of imputing missing values using bootstrapping
...
#'
get_all_performance_boot = function(data, to_impute, regressors, nb = 1) {
  # Function to evaluate different nan imputation methods with bootstrap.
  # Input:
  ...
  # 4) nb: number of bootstrap samples.
  # It returns:
  # 1) performance: dataframe with rows = methods and columns = methods' performances averaged over bootstraps.
  performance <- data.frame()
  for (method in 1:5) {
    method_performances <- matrix(NA, nrow = nb, ncol = 3)
    for (boot in 1:nb) {
      boot_performance <- performance_nan_imputation(data, to_impute, regressors, method = method)
      method_performances[boot, ] <- colMeans(boot_performance, na.rm = TRUE)
    }
    average_performance <- colMeans(method_performances, na.rm = TRUE)
    performance <- rbind(performance, average_performance)
  }
  ...
  colnames(performance) <- c("R2", "rmse", "mae")
